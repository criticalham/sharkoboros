package  
{
	 ...
	 * @author 
	 */
	public class DummyEnemy extends Character
	{
		...
		public var dummyPosition:FlxPoint;
		 ...
		 * @param	Y
		 */
		public function DummyEnemy(X:Number, Y:Number, boss:Boolean = false): void
		{
			dummyPosition = new FlxPoint (FlxG.random() * FlxG.width, FlxG.random() * FlxG.height);
			isBoss = boss;
			super(X, Y);
		...
		override public function update(): void
		{
			...
			else
			{
				if (FlxG.random() < 0.001)
				{
					var randBool:Boolean = FlxG.random() > 0;
					dummyPosition = new FlxPoint (randBool ? 0 : FlxG.random() * FlxG.width, randBool ? FlxG.random() * FlxG.height : 0);
				}
			}
			play("default");
		...
		public function idleState(): void
		{
			...
			var direction:FlxPoint = VecUtil.subtract(dummyPosition, position);
